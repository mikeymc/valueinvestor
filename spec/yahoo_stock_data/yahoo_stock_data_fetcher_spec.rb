...
RSpec.describe YahooStockDataFetcher do
  it 'returns data given a list of ticker symbols' do
    ...
    first_stock_dividend_yield = 1.23
    ...
    second_stock_dividend_yield = 2.34
    ...
    aapl_xom_fake_response << "#{first_stock_dividend_yield}\n"
    ...
    aapl_xom_fake_response << "#{second_stock_dividend_yield}\n"

    WebMock
      .stub_request(:get, 'http://download.finance.yahoo.com/d/quotes.csv?f=nsxe7dhgb4p6rjkl1j4j1t8m3m8m4m6y&s=TIF%2BAAPL')
    ...
    expect(first_stock[:ebitda]).to eq(1010000000)
    expect(first_stock[:last_trade_price]).to eq(65.12)
    expect(first_stock[:market_cap]).to eq(8350000000)
    ...
    expect(first_stock[:dividend_yield]).to eq(1.23)
    ...
    expect(second_stock[:ebitda]).to eq(82790000000)
    expect(second_stock[:last_trade_price]).to eq(96.76)
    expect(second_stock[:market_cap]).to eq(536490000000)
    ...
    expect(second_stock[:dividend_yield]).to eq(2.34)
  end
end

...
RSpec.describe StockDataJoiner do
  it 'merges the Yahoo data into the stocks' do
    ...
    second_stock[:current_eps] = '-0.190'
    ...
    second_stock[:price_to_earnings_ratio] = 'N/A'
    second_stock[:year_low_price] = 'N/A'
    second_stock[:year_high_price] = 'N/A'
    ...
    expect(rgdx_stock[:current_eps]).to eq('-0.190')
    ...
    expect(rgdx_stock[:price_to_earnings_ratio]).to eq('N/A')
    expect(rgdx_stock[:year_low_price]).to eq('N/A')
    expect(rgdx_stock[:year_high_price]).to eq('N/A')
  end
end

...
RSpec.describe StockDataJoiner do
  it 'merges the Yahoo data into the stocks' do
    ...
    expect(tif_stock[:current_eps]).to eq(3.78)
    expect(tif_stock[:dividends_per_share]).to eq(1.6)
    ...
    expect(tif_stock[:price_to_earnings_ratio]).to eq(44.44)
    expect(tif_stock[:year_low_price]).to eq(100.11)
    expect(tif_stock[:year_high_price]).to eq(200.11)

    expect(rgdx_stock[:name]).to eq('The Gap')
    expect(rgdx_stock[:symbol]).to eq('RGDX')
    expect(rgdx_stock[:exchange]).to eq('Nasdaq')
    expect(rgdx_stock[:current_eps]).to eq(-0.19)
    expect(rgdx_stock[:dividends_per_share]).to eq(nil)
    expect(rgdx_stock[:day_high_price]).to eq(nil)
    expect(rgdx_stock[:day_low_price]).to eq(nil)
    expect(rgdx_stock[:book_value]).to eq(0.0)
    expect(rgdx_stock[:price_to_book_ratio]).to eq(nil)
    expect(rgdx_stock[:price_to_earnings_ratio]).to eq(nil)
    expect(rgdx_stock[:year_low_price]).to eq(nil)
    expect(rgdx_stock[:year_high_price]).to eq(nil)
  end
end
